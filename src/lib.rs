...
pub fn main() -> Result<(), Box<dyn Error>> {
    xmain(EventLoopBuilder::new().build().unwrap())
}

fn xmain(event_loop: winit::event_loop::EventLoop<()>) -> Result<(), Box<dyn Error>> {
    let transparent_window = false;
    ...
    let window_builder = if cfg!(wgl_backend) {
        Some(WindowBuilder::new().with_transparent(transparent_window))
    } else {
        None
    };
        ...
        match event {
            Event::Resumed => {
                ...
                let window = window.take().unwrap_or_else(|| {
                    let window_builder = WindowBuilder::new().with_transparent(transparent_window);
